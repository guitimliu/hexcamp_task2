...
function Works() {
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-2 md:px-0 mb-10 md:mb-20">
        <ul className="flex justify-between gap-y-20 flex-wrap mb-12">
            <li className="w-full md:w-[48%]">
              <div onClick={() => setShowPostID(1)} className="cursor-pointer">
                <img className="rounded-t-xl mb-4 max-w-full" src="https://fakeimg.pl/636x400/200" />
                <div className="pb-4">
      ...
      {
        showPostID !== 0 ?
        <div className="flex justify-center absolute top-0 left-0 w-full min-h-full">
          <div 
            className="absolute top-0 left-0 z-20 w-full h-full bg-black opacity-75"
            onClick={() => setShowPostID(0)}
          />
          <div className="relative bg-white max-w-[1200px] rounded-xl my-20 z-20">
              <div className="pt-10 pr-10">
                <svg onClick={() => setShowPostID(0)}  className="cursor-pointer ms-auto" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
